...
import CarbonCalc from './CarbonCalc.jsx';
...
export default function DailyQuestions({ returningUser, currentUser }) {
  ...
  const answers = {
    ...
    1: [
      'Increased the use of public transportation or carpooling',
      'Increased personal car usage',
      'No change in transportation habits',
    ],
    2: ['0'],
    3: [
      'Increased my recycling efforts',
      'Maintained consistent recycling habits',
      'Decreased my recycling efforts',
    ],
    4: ['Yes, I compost regularly', 'I rarely compost', 'No, I do not compost'],
    5: [
      'Reduced meat consumption',
      'Transitioned to a vegetarian diet',
      'Transitioned to a vegan diet',
      'No significant diet changes',
    ],
    6: ['0'],
    7: [
      'Yes, I actively support and engage in local sustainability initiatives',
      'I am aware of such initiatives but have not participated',
      'No, I am not involved in local sustainability initiatives',
    ],
  };
  ...
  const updateUserScores = (updatedUserScores) => {
    axios
      .put(`${API}/users/scores/${currentUser.user_auth_id}`, updatedUserScores)
      .then((response) => {
  ...
  const updateQuestionAnswers = (updatedAnswersTable) => {
    axios
      .put(
        `${API}/users/answers/${currentUser.user_auth_id}`,
        updatedAnswersTable,
      )
      .then((response) => {
        console.log(response);
  ...
  const handleAnswersTable = () => {
    const updatedQuestionAnswers = {
      ...existingAnswers,
      'Select the option that best describes your household energy usage compared to your last login':
        selectedAnswers[0],
      'Pick the option that best represents your daily transportation habits compared to your last login':
        selectedAnswers[1],
      ...
      'Do you support or engage in local community initiatives promoting sustainability?':
        selectedAnswers[7],
    };
  ...
  const handleUserScore = () => {
    ...
    if (selectedAnswers[3] === 'Increased my recycling efforts') {
      ...
      selectedAnswers[3] === 'Maintained consistent recycling habits'
    ) {
      recyclingScore = 100;
    }
            ...
            >
              <div className="bg-green-300 px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                ...
                <form
                  ...
                  className="space-y-4 border-8 border-gray-300 bg-white p-8 text-center"
                >
                  <motion.div
                    key={questNum}
                    initial={{ opacity: 0, y: -20 }}
                      ...
                      if (questNum === 2) {
                        return (
                          <div key={index}>
                            <input
                              ...
                              max="100"
                              value={milesBiked}
                              className="h-2 w-full rounded-lg bg-blue-500 md:w-80"
                              onChange={(e) => {
                        ...
                        return (
                          <div key={index}>
                            <button
                              type="button"
                              className="group relative mb-2 mr-2 inline-flex items-center justify-center overflow-hidden rounded-lg bg-gradient-to-br from-teal-300 to-lime-300 p-0.5 text-sm font-medium text-gray-900 focus:outline-none focus:ring-4 focus:ring-lime-200 active:bg-gradient-to-br active:from-lime-300 active:to-lime-300"
                              onClick={() => handleAnswerSelection(ans)}
                            >
                              <span className="relative rounded-md bg-white px-5 py-2.5 transition-all duration-75 ease-in group-hover:bg-opacity-0 group-focus:bg-opacity-0 group-focus:text-gray-900">
                                {ans}
                              </span>
                            </button>
                          </div>
