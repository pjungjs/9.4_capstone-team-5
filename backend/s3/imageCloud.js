...
const { uploadToS3, getUserImagesInfo } = require('./s3.js');

// upload an image to AWS S3
imageCloud.post('/:userAuthId', upload.single('image'), async (req, res) => {
  const { file } = req;
  ...
  const { success, payload } = await uploadToS3({ file, userAuthId });
...
imageCloud.get('/:userAuthId', async (req, res) => {
  const { userAuthId } = req.params;
